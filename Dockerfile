...
ARG LUA_RESTY_HMAC_VERSION=0.06
...
RUN apt-get update \
  ...
  && curl --fail --silent --show-error --location \
       "https://raw.githubusercontent.com/jkeys089/lua-resty-hmac/v${LUA_RESTY_HMAC_VERSION}/lib/resty/hmac.lua" \
       --output /usr/local/share/lua/5.1/resty/hmac.lua \
  ...
  && grep -q "_VERSION = '${LUA_RESTY_HMAC_VERSION}'" \
       /usr/local/share/lua/5.1/resty/hmac.lua \
